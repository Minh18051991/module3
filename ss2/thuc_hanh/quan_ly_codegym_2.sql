...
SELECT cl.name , COUNT(*) as student_count
FROM class cl
JOIN student st on cl.id = st.class_id
GROUP BY cl.name;

-- Tính điểm lớn nhất của mỗi các lớp
SELECT cl.name, MAX(st.point) as max_point
FROM class cl
JOIN student st ON cl.id = st.class_id
GROUP BY cl.name;
-- Tinh điểm trung bình của oỗi lớp
SELECT cl.name, AVG(st.point) as avg_point
FROM class cl
JOIN student st ON cl.id = st.class_id
GROUP BY cl.name;

-- tên và ngày sinh của giáo viên và học sinh của codegym
SELECT st.name, st.birthday, instructor.name as instructor_name,instructor.birthday as birthday
FROM student st
JOIN instructor on instructor.id = st.id ;
-- Top 3 sinh viên có điểm cao nhât
...
FROM instructor
WHERE instructor.id NOT IN (SELECT instructor_id FROM instructor_class);
